...
Aplicación web (React + TypeScript + Vite + Tailwind) de la Plataforma de Análisis de Ingreso y Patrones de Consumo Minorista. Consume el backend monolito de `api/`.
...
**Importante:** el backend (`api`) debe estar corriendo antes de probar el login (`npm run start:dev` dentro de esa carpeta), y con CORS habilitado — ya lo está por defecto (`app.enableCors()` en `main.ts`).
